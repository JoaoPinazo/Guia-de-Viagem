...
const PartyCard: React.FC<PartyCardProps> = ({ party, onClick, isFavorite, onToggleFavorite }) => {
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-48">
        <ImageWithLoader
          imageQuery={party.imagem_query}
          altText={`Imagem de ${party.nome}`}
          className="w-full h-full object-cover"
        />
      </div>
      <div className="p-5 flex flex-col flex-grow">
        <h3 className="text-2xl font-bold text-white mb-2">{party.nome}</h3>
        <p className="text-gray-300 mb-4 flex-grow">{party.descricao}</p>
        <div className="flex flex-col gap-3 mb-4">
            <div className="flex items-center text-blue-400">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd" />
                </svg>
                <span className="font-medium">{party.data_evento}</span>
            </div>
            <div className="flex items-start text-blue-400">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 flex-shrink-0 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
                </svg>
                <span className="font-medium">{party.endereco}</span>
            </div>
            <div className="flex items-center text-blue-400">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                ...
                <span className="font-medium">{party.preco_ingresso}</span>
            </div>
        </div>
         {party.atracoes && party.atracoes.length > 0 && (
          <div className="border-t border-gray-700 pt-4">
              <h4 className="text-lg font-semibold text-white mb-3 flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                  <path d="M3.5 13.5a1.5 1.5 0 113 0 1.5 1.5 0 01-3 0zM12 15.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3z" />
                  <path d="M17 3a1 1 0 00-1-1h-2a1 1 0 00-1 1v8a1 1 0 001 1h2a1 1 0 001-1V3zM6 3a1 1 0 00-1-1H3a1 1 0 00-1 1v8a1 1 0 001 1h2a1 1 0 001-1V3z" />
                </svg>
                Atrações Principais
              </h4>
              <ul className="space-y-2 text-gray-300 list-disc list-inside">
                {party.atracoes.map((item, index) => (
